...
from state_pattern.generator.state_spec import StateSpec
from state_pattern.generator.store import Store
from state_pattern.generator.transition_spec import TransitionSpec
...
class WorkflowGenerator:
    ...
    def generate(self) -> None:
        ...
        while len(opens) > 0:
            ...
            if isinstance(task, StartEvent) and task.name == "s1_a1_start_event":
                opens.union(set(self.travel_states(task)))
            closes.add(task)
        ...
        self.store.save_path_enum(self.transitions)
        self.store.save_nodes(self.state_specs)
        self.store.save_node_links(self.transitions)
...
#     nodes = json.load(open("./code/nodes.json", "r", encoding="utf8"))
