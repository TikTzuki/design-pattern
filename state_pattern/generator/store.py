...
class Store:
    ...
    def save_class(self, states: Iterable[StateSpec]):
        ...
        for state in states:
            ...
            with functions.op(f"{file_path}.py", "w") as f:
                f.write(
                    "\n".join([
    ...
    def save_transition_enum(self, transitions: Iterable[TransitionSpec]):
        with functions.op(f"{self.const_dir}/transition.py", "w") as f:
            f.write(
                "from enum import auto\n"
                + "from fastapi_utils.enums import StrEnum\n"
                + "class ETransition(StrEnum):\n"
                + "\n".join(sorted(set(map(lambda t: f"\t{t.id} = auto()\n\t\"{t.status_out_message}\"", transitions))))
            )

    def save_state_enum(self, states: Iterable[StateSpec]):
        with functions.op(f"{self.const_dir}/state.py", "w") as f:
            f.write(
                "from enum import auto\n"
                + "from fastapi_utils.enums import StrEnum\n"
